...
def _resolve_order_run_id(session: Session, portfolio_id: int) -> int:
    env_id = _get_env_int("M6_ORDER_RUN_ID")
    if env_id is not None:
        exists = session.execute(
            text(
                """
        ...
        if exists is None:
            raise ValueError(
                f"M6_ORDER_RUN_ID={env_id} has no ACCEPTED orders "
                f"for portfolio_id={portfolio_id}."
            )
        return env_id

    order_run_id = session.execute(
        text(
            """
            select run_id
            from trading_paper_order
    ...
    if order_run_id is None:
        raise ValueError(
            "cannot resolve order_run_id. "
            "Run bootstrap_m6_paper_order_chain first or set M6_ORDER_RUN_ID."
        )

    return int(order_run_id)
    ...
    return int(
        session.execute(
            sql,
            {
                ...
                "run_type": "paper_fill",
                "run_name": "M6 Paper Fill First Chain",
                "status": "RUNNING",
                "trigger_type": "MANUAL",
...
def main() -> None:
    ...
    with _open_session() as session:
        ...
        order_run_id = _resolve_order_run_id(session, portfolio_id=portfolio_id)
        ...
        result = simulate_paper_fills(session=session, request=request)
        _mark_ops_run_success(session=session, run_id=fill_run_id)

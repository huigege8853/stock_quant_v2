...
def _fetch_baostock_adjust_factor(_api_client, exchange_code: str, ticker: str, trade_date: date) -> list[AdjustFactorDTO]:
    ...
    return [
        _normalize_adjust_factor_row("baostock", exchange_code, ticker, trade_date, row)
        for row in rows
    ]


def _fetch_akshare_adjust_factor(_api_client, exchange_code: str, ticker: str, trade_date: date) -> list[AdjustFactorDTO]:
    if _api_client is None:
        return []
    ...
    return [
        _normalize_adjust_factor_row("akshare", exchange_code, ticker, trade_date, row)
        for row in rows
    ]


def _fetch_tushare_adjust_factor(_api_client, exchange_code: str, ticker: str, trade_date: date) -> list[AdjustFactorDTO]:
    if _api_client is None:
        return []
    ...
    return [
        _normalize_adjust_factor_row("tushare", exchange_code, ticker, trade_date, row)
        for row in rows
    ]


def _fetch_symbol_adjust_factor(
    *,
    ...
    for provider_name in _provider_priority():
        ...
        if provider_name == "baostock":
            provider_attempts.append(
                ("baostock", lambda: _fetch_baostock_adjust_factor(baostock_api_client, exchange_code, ticker, trade_date))
            )
        elif provider_name == "akshare":
            provider_attempts.append(
                ("akshare", lambda: _fetch_akshare_adjust_factor(akshare_api_client, exchange_code, ticker, trade_date))
            )
        elif provider_name == "tushare":
            provider_attempts.append(
                ("tushare", lambda: _fetch_tushare_adjust_factor(tushare_api_client, exchange_code, ticker, trade_date))
            )
        elif provider_name == "paid":
            provider_attempts.append(("paid", lambda: []))
        elif provider_name == "sina":
            provider_attempts.append(("sina", lambda: []))
...
def run_backfill_adjust_factor(
    session: Session,
    baostock_api_client,
    akshare_api_client,
    tushare_api_client,
    ...
    for chunk_no, chunk_dates in enumerate(date_chunks, start=1):
        ...
        data_sync_run = sync_repo.create_data_sync_run(
            ...
            {
                "run_id": run_id,
                "sync_job_code": "backfill_adjust_factor",
        ...
        try:
            ...
            progress = tqdm(
                chunk_dates,
                desc=f"backfill_adjust_factor {chunk_start.isoformat()}~{chunk_end.isoformat()}",
                unit="day",
                dynamic_ncols=True,
            )
            ...
            for trade_date in progress:
                ...
                def _worker(item: dict) -> BackfillFetchResult:
                    return _fetch_symbol_adjust_factor(
                ...
                for result in results:
                    ...
                    for row in result.rows:
                        dto = AdjustFactorDTO(
                            ...
                            adjust_factor=row.get("adjust_factor"),
                ...
                coverage_ratio = round(day_hit_symbols / len(universe), 6) if universe else 0.0

...
def dto_to_raw_adjust_factor_dict(dto: AdjustFactorDTO, sync_run_id: int, batch_id: int) -> dict:
...
def dto_to_staging_adjust_factor_dict(
    dto: AdjustFactorDTO,
    sync_run_id: int,
    batch_id: int,
    ...
    return {
        ...
        "trade_date": dto.trade_date,
        "adjust_factor": dto.adjust_factor,
        "provider_record_key": dto.provider_record_key,
        "raw_record_id": raw_record_id,
    }


def staging_to_core_adjust_factor_dict(stg_row: dict, instrument_id: int, data_version_id: int) -> dict:
    now = datetime.utcnow()
    factor = stg_row.get("adjust_factor")
